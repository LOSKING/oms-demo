...
3. **主流企业级系统集成模拟**：
   - **WMS (三方接口)**：模拟三方标准的入库/出库单据创建与收发货确认回调。
...
### 1. B2B 采购直发 WMS 流程
中台新建采购订单 (PO) 推送 SRM，供应商确认发货后由 OMS 创建三方入库单推至顺丰 WMS，WMS 收货确认后回传中台，自动完成采购单并增加 WMS 仓库存。
...
sequenceDiagram
    ...
    OMS->>WMS: 3. 下发三方入库单 (3rdParty Inbound)
    WMS->>OMS: 4. 入库上架确认回调
